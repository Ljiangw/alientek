...
#include <stdint.h>
#define DT_DRV_COMPAT alientek_stm32_qspi_nor
...
LOG_MODULE_REGISTER(flash_alientek_qspi, CONFIG_FLASH_LOG_LEVEL);
...
#if DT_HAS_COMPAT_STATUS_OKAY(alientek_stm32_qspi_nor)
...
#endif
#if DT_NODE_HAS_PROP(DT_INST(0, alientek_stm32_qspi_nor), jedec_id)
...
static int flash_stm32_qspi_init(const struct device *dev)
{
    ...
    uint8_t sr = 0;
    qspi_read_status_register(dev, 2U, &sr);
    LOG_INF("Status Register 2: 0x%02x", sr);
    qspi_read_status_register(dev, 3U, &sr);
    LOG_INF("Status Register 3: 0x%02x", sr);

    const uint8_t g_text_buf[] = {"STM32H7 QSPI TEST"};
    uint8_t rectemp[sizeof(g_text_buf) + 2] = {0};
    ...
    HAL_QSPI_Command(&dev_data->hqspi, &cmd,
				   HAL_QSPI_TIMEOUT_DEFAULT_VALUE);
    ...
    ret = HAL_QSPI_Command(&dev_data->hqspi, &cmd,
				   HAL_QSPI_TIMEOUT_DEFAULT_VALUE);
	if (ret != HAL_OK) {
		LOG_ERR("%d: Failed to send SFDP instruction", ret);
	}

    uint8_t temp[2];
    uint16_t deviceid;

	ret = HAL_QSPI_Receive(&dev_data->hqspi, (uint8_t *)temp,
				   HAL_QSPI_TIMEOUT_DEFAULT_VALUE);
	if (ret != HAL_OK) {
		LOG_ERR("%d: Failed to read SFDP", ret);
	}
    deviceid = (temp[0] << 8) | temp[1];
    LOG_INF("deviceid:0x%04x  \r\n", deviceid);

    ret = flash_stm32_qspi_write(dev, 0x1000, g_text_buf, sizeof(g_text_buf));
    if (ret != 0) {
        LOG_ERR("flash write failed: %d", ret);
        // return -EIO;
    }

    ret = flash_stm32_qspi_read(dev, 0x1000, rectemp, sizeof(g_text_buf));
    if (ret != 0) {
